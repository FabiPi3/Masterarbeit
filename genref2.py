...
'''
This code computes the referece structure. It consists of two parts.
First the structure is relaxed by MD.
Second the needed information are extracted.
'''
...
view(atoms)
# the next two lines are only needed if you run the 2 parts seperately
#write('res_structs/genref3.cfg', atoms)
#atoms=read('res_structs/genref3.cfg')

# define parameters for the electron diffraction simulation 
# (see the according function in soaputils for further information)
a_g=3*a
reso=0.15
sample = int(a_g/reso)
atoms_g = atoms*(3,3,2)
...
# save the correlographs
np.save('corAvg3', correlographAvg)
np.save('corVar3', correlographVar)

# display the correlograph mean and its variance
fig = p.figure(figsize=(64, 48))
p.imshow(np.log(1+np.abs(correlographAvg)),extent=[kmin,kmax,2*np.pi,0])
...
fig = p.figure(figsize=(64, 48))
p.imshow(np.log(1+np.abs(correlographVar)),extent=[kmin,kmax,2*np.pi,0])
p.axis('auto'); p.axis('tight'); p.colorbar()
p.xlabel('k in inversen Angstrom')
p.ylabel('Winkel')
p.show()

# display angles distribution and RDF
